    ...
    <div className="bg-slate-50 min-h-screen pb-20 no-print font-sans relative">
          ...
          {activeTab === 'lyrics' && (
            <div className="space-y-12">
              <div className="bg-indigo-50/50 p-6 rounded-2xl border border-indigo-100 flex items-center gap-4">
                ...
                <p className="text-indigo-600 text-sm font-medium">在此编辑句段。勾选右侧“选为作业”后，该句将出现在学生的复习练习中。</p>
                <button onClick={() => setShowBatchModal(true)} className="ml-auto bg-indigo-600 text-white px-4 py-2 rounded-lg text-xs font-bold">批量导入</button>
              </div>
                ...
                <div key={line.id} className="flex gap-8 relative pb-10 border-b border-slate-50 last:border-0 group">
                  <div className="w-24 flex flex-col items-center gap-3">
                    ...
                    <div className="space-y-1.5 w-full">
                      <input type="number" className="w-full bg-slate-50 border border-slate-100 rounded-lg p-1.5 text-center text-[11px] font-bold outline-none" value={line.startTime} onChange={(e) => setLesson({...lesson, lyrics: lesson.lyrics.map(l => l.id === line.id ? {...l, startTime: Number(e.target.value)} : l)})} />
                      <input type="number" className="w-full bg-slate-50 border border-slate-100 rounded-lg p-1.5 text-center text-[11px] font-bold outline-none" value={line.endTime} onChange={(e) => setLesson({...lesson, lyrics: lesson.lyrics.map(l => l.id === line.id ? {...l, endTime: Number(e.target.value)} : l)})} />
                    </div>
                  </div>
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    <input className="w-full text-lg text-slate-400 placeholder-slate-100 focus:outline-none font-medium" placeholder="拼音/Jyutping..." value={line.pinyin} onChange={(e) => setLesson({...lesson, lyrics: lesson.lyrics.map(l => l.id === line.id ? {...l, pinyin: e.target.value} : l)})} />
          ...
          {activeTab === 'questions' && (
            <div className="space-y-8">
              <div className="flex justify-between items-center mb-8">
                <div>
                  <h3 className="text-2xl font-black text-slate-800 tracking-tight">课堂互动题</h3>
                  <p className="text-slate-400 text-sm mt-1">设置视频播放中弹出的随堂练习</p>
                </div>
              ...
              {lesson.questions.length === 0 && (
                <div className="py-20 text-center border-4 border-dashed border-slate-50 rounded-[3rem]">
                   <i className="fa-solid fa-clipboard-question text-6xl text-slate-100 mb-4"></i>
                   <p className="text-slate-300 font-bold text-lg">目前还没有设置任何习题</p>
                </div>
              )}
              ...
              {lesson.questions.map((q, idx) => (
                <div key={q.id} className="bg-slate-50 p-8 rounded-[2rem] border-2 border-slate-100 space-y-6">
                  <div className="flex gap-4 items-start">
                    <div className="w-24">
                       <label className="text-[10px] font-black text-slate-400 block mb-1">触发时间(秒)</label>
                       <input type="number" className="w-full p-3 rounded-xl border-2 border-white outline-none focus:border-indigo-400 font-mono shadow-inner" value={q.timestamp} onChange={(e) => setLesson({...lesson, questions: lesson.questions.map(item => item.id === q.id ? {...item, timestamp: Number(e.target.value)} : item)})} />
                    </div>
                    <div className="flex-1">
                       <label className="text-[10px] font-black text-slate-400 block mb-1">问题内容</label>
                       <input className="w-full p-3 rounded-xl border-2 border-white outline-none focus:border-indigo-400 font-bold shadow-inner" value={q.question} onChange={(e) => setLesson({...lesson, questions: lesson.questions.map(item => item.id === q.id ? {...item, question: e.target.value} : item)})} />
                    </div>
                    <button onClick={() => setLesson({...lesson, questions: lesson.questions.filter(item => item.id !== q.id)})} className="text-slate-200 hover:text-red-400 p-3 mt-4"><i className="fa-solid fa-trash-can"></i></button>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    {q.options.map((opt, oIdx) => (
                      <div key={oIdx} className="flex items-center gap-3">
                        <input type="radio" name={`q-${q.id}`} checked={q.correctIndex === oIdx} onChange={() => setLesson({...lesson, questions: lesson.questions.map(item => item.id === q.id ? {...item, correctIndex: oIdx} : item)})} className="w-5 h-5 accent-emerald-500" />
                        <input className="flex-1 p-3 rounded-xl border-2 border-white text-sm outline-none focus:border-indigo-400 shadow-inner" placeholder={`选项 ${oIdx + 1}`} value={opt} onChange={(e) => {
                        ...
                        {q.options.length > 2 && (
                          <button onClick={() => {
                            ...
                            setLesson({...lesson, questions: lesson.questions.map(item => item.id === q.id ? {...item, options: newOpts, correctIndex: 0} : item)});
                          }} className="text-slate-200 hover:text-red-400"><i className="fa-solid fa-circle-minus"></i></button>
                        )}
                      </div>
                    ))}
                    <button onClick={() => setLesson({...lesson, questions: lesson.questions.map(item => item.id === q.id ? {...item, options: [...item.options, '']} : item)})} className="text-xs font-black text-indigo-400 border-2 border-dashed border-indigo-200 rounded-xl py-4 hover:bg-indigo-50 transition-all">+ 添加新选项</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {activeTab === 'students' && (
            <div className="space-y-12">
               <div className="bg-slate-50 p-8 rounded-[2rem] border-2 border-slate-100">
                 ...
                 <p className="text-xs text-slate-400 mb-4 font-medium italic">上传 JSON 文件到您的 GitHub 仓库后，请填入仓库链接以便学生自动同步。</p>
                 <div className="flex gap-4">
                    <input className="flex-1 p-4 rounded-xl border-2 border-white outline-none font-mono text-xs shadow-inner" placeholder="例如: https://blublug1012.github.io/homework-data/" value={cloudBaseUrl} onChange={(e) => setCloudBaseUrl(e.target.value)} />
                    <button onClick={() => alert("✅ 云端配置已保存")} className="bg-indigo-600 text-white px-8 py-4 rounded-xl font-black text-xs">保存</button>
                 </div>
               </div>
               ...
               <div className="flex justify-between items-center">
                 ...
                 }} className="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-bold shadow-lg shadow-indigo-100">+ 添加学生</button>
               </div>

               <div className="grid grid-cols-2 gap-8">
                 {db.students.map(s => (
                   <div key={s.id} className="bg-white border-2 border-slate-100 p-8 rounded-[2.5rem] group hover:border-indigo-200 transition-all">
                     <h4 className="text-2xl font-black text-slate-800">{s.name}</h4>
                     <p className="text-[10px] font-black text-slate-300 mt-1 uppercase">ID: {s.id}</p>
                     <div className="mt-8 space-y-3">
                       <button onClick={() => {
                          const newDb = {...db};
                          const student = newDb.students.find(item => item.id === s.id);
                          if(student && !student.assignedLessons.includes(lesson.id)) {
                             student.assignedLessons.push(lesson.id);
                             setDb(newDb); localStorage.setItem('teaching_db', JSON.stringify(newDb));
                             alert(`已指派课程《${lesson.title}》`);
                          } else {
                             alert("该课程已存在");
                          }
                       ...
                       <button onClick={() => exportStudentPackage(s.id)} className="w-full bg-emerald-500 text-white py-4 rounded-2xl font-black text-sm flex items-center justify-center gap-2 shadow-lg shadow-emerald-50">
                         <i className="fa-solid fa-file-export"></i> 导出作业 JSON
                       </button>
                     </div>
                   </div>
                 ))}
          ...
          {activeTab === 'info' && (
            <div className="space-y-8 max-w-xl mx-auto py-10">
              <div className="space-y-2">
                ...
                <input className="w-full p-5 bg-slate-50 rounded-2xl text-xl font-black outline-none border-2 border-transparent focus:border-indigo-400 shadow-inner" value={lesson.title} onChange={(e) => setLesson({...lesson, title: e.target.value})} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-300 uppercase tracking-widest ml-1">YouTube 视频链接</label>
                <input className="w-full p-5 bg-slate-50 rounded-2xl outline-none border-2 border-transparent focus:border-indigo-400 font-mono text-indigo-600 shadow-inner" value={lesson.videoUrl} onChange={(e) => setLesson({...lesson, videoUrl: e.target.value})} />
              </div>
            </div>
          )}
        </div>
      ...
      <div className="hidden print-container">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400 mt-2 font-medium tracking-widest">学习讲义 / STUDY HANDOUT</p>
        </div>
        {lesson.lyrics.map((line, idx) => (
          <div key={line.id} className="lyric-item pb-10 border-b-2 mb-10">
            <div className="flex flex-wrap gap-x-10 gap-y-12 mb-8">
              {Array.from(line.chinese.replace(/\s/g,'')).map((c, i) => (
                <div key={i} className="flex flex-col items-center">
                  <span className="text-sm text-slate-400 font-bold mb-2 uppercase tracking-tighter">{line.pinyin.split(/\s+/)[i] || ''}</span>
                  <span className="text-5xl font-bold pb-2">{c}</span>
                </div>
              ))}
            </div>
            <p className="italic text-slate-500 text-2xl mb-6">"{line.english}"</p>
            {line.vocabs.length > 0 && (
              <div className="grid grid-cols-2 gap-4 bg-slate-50 p-6 rounded-2xl">
                {line.vocabs.map((v, vi) => (
                  <div key={vi} className="text-lg">
                    <span className="font-bold text-indigo-600">{v.char}</span>
                    <span className="text-slate-400 ml-2">({v.pinyin})</span>
                    <span className="ml-4 text-slate-700 font-medium">{v.explanation}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {showBatchModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex items-center justify-center p-6">
           <div className="bg-white rounded-[3rem] p-12 max-w-2xl w-full shadow-2xl">
              ...
              <textarea className="w-full h-72 p-8 bg-slate-50 rounded-[2rem] border-2 border-slate-100 focus:outline-none focus:border-indigo-400 font-medium text-lg shadow-inner" placeholder="在此粘贴多行文本..." value={batchText} onChange={(e) => setBatchText(e.target.value)}></textarea>
